...
namespace cslam
{
    ...
    void ServerSystem::InitializeMaps()
    {
        ...
        if (mNumOfClients > 1)
            mpMap1.reset(new Map(mNh, mNhPrivate, 1, eSystemState::SERVER));
        else
            mpMap1 = nullptr;
        if (mNumOfClients > 2)
            mpMap2.reset(new Map(mNh, mNhPrivate, 2, eSystemState::SERVER));
        else
            mpMap2 = nullptr;
        if (mNumOfClients > 3)
            mpMap3.reset(new Map(mNh, mNhPrivate, 3, eSystemState::SERVER));
        else
            mpMap3 = nullptr;
    }
    ...
    void ServerSystem::InitializeMapMatcher()
    {
        ...
        if (mpClient0)
            mpClient0->SetMapMatcher(mpMapMatcher);
        if (mpClient1)
            mpClient1->SetMapMatcher(mpMapMatcher);
        if (mpClient2)
            mpClient2->SetMapMatcher(mpMapMatcher);
        if (mpClient3)
            mpClient3->SetMapMatcher(mpMapMatcher);
    }
