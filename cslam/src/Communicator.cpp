...
namespace cslam
{
          ...
          mbLoadedMap(bLoadedMap)
    {
        ...
        if (mbLoadedMap)
            return; // do not register communication infrastructure (publisher/subscriber) when map is loaded
    ...
    void Communicator::ProcessKfInClient()
    {
        ...
        while (!mlBufKFin.empty() && (ItCount < mKfItBound))
        {
            ...
            {
                cout << "\033[1;34m!!! NOTICE !!!\033[0m " << __func__ << ":" << __LINE__ << " -- Agent " << mpCC->mClientId << ": reached INPUT KF iteration limit"
                     << "[" << mlBufKFin.size() << "]" << endl;
            }
#endif
        }
    ...
    void Communicator::ProcessMpInClient()
    {
        ...
        while (!mlBufMPin.empty() && (ItCount < mMpItBound))
        {
            ...
            {
                cout << "\033[1;34m!!! NOTICE !!!\033[0m " << __func__ << ":" << __LINE__ << " -- Agent " << mpCC->mClientId << ": reached INPUT MP iteration limit"
                     << "[" << mlBufMPin.size() << "]" << endl;
            }
#endif
        }
