...
namespace cslam
{
    ...
    KeyFrameDatabase::KeyFrameDatabase(const vocptr pVoc) : mpVoc(pVoc)
    {
        ...
        // Discard keyframes connected to the query keyframe
        {
            ...
            for (DBoW2::BowVector::const_iterator vit = pKF->mBowVec.begin(), vend = pKF->mBowVec.end(); vit != vend; vit++)
            {
                ...
                for (list<kfptr>::iterator lit = lKFs.begin(), lend = lKFs.end(); lit != lend; lit++)
                {
                    ...
                    if (mit == mpAllKfsInMap.end())
                        continue; // Only consider KFs that belong to the same map
