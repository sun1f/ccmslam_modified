...
namespace cslam
{
          ...
          mbAck(false), mbFromServer(false), mbUpdatedByServer(false), mCorrected_MM(defpair), mbSendFull(true)
    {
        ...
        if (mUniqueId == 0 && !mId.first == 0)
            cout << "\033[1;31m!!!!! ERROR !!!!!\033[0m KeyFrame::KeyFrame(..): mUniqueId not set" << endl;
    ...
    void KeyFrame::EraseMapPointMatch(const size_t &idx, bool bLock)
    {
        unique_lock<mutex> lock(mMutexFeatures);
        if (this->IsEmpty())
            cout << "\033[1;31m!!!!! ERROR !!!!!\033[0m KeyFrame::EraseMapPointMatch(): trying to erase MP from empty KF" << endl;
    ...
    void KeyFrame::ReplaceMapPointMatch(const size_t &idx, mpptr pMP, bool bLock, bool bOverrideLock)
    {
        ...
        if (mvpMapPoints[idx] && mvpMapPoints[idx]->mbDoNotReplace)
            cout << "\033[1;31m!!!!! ERROR !!!!!\033[0m KeyFrame::ReplaceMapPointMatch(...): mbDoNotReplace true" << endl;
    ...
    void KeyFrame::ConvertToMessage(ccmslam_msgs::Map &msgMap, g2o::Sim3 mg2oS_wcurmap_wclientmap, kfptr pRefKf, bool bForceUpdateMsg)
    {
        ...
        if ((mbSendFull || mSysState == eSystemState::SERVER) && !bForceUpdateMsg)
        {
            ...
            for (int idx = 0; idx < mvScaleFactors.size(); ++idx)
                Msg.mvScaleFactors[idx] = mvScaleFactors[idx];
            for (int idx = 0; idx < mvLevelSigma2.size(); ++idx)
                Msg.mvLevelSigma2[idx] = mvLevelSigma2[idx];
            for (int idx = 0; idx < mvInvLevelSigma2.size(); ++idx)
                Msg.mvInvLevelSigma2[idx] = mvInvLevelSigma2[idx];
