...
namespace cslam
{
    ...
    Frame::Frame(const cv::Mat &imGray, const cv::Mat &imDepth, const double &timeStamp, extractorptr pExtractor, vocptr pVoc, cv::Mat &K, cv::Mat &distCoef, const float &bf, const float &thDepth, size_t ClientId)
        : mpORBvocabulary(pVoc), mpORBextractor(pExtractor), mTimeStamp(timeStamp), mK(K.clone()), mDistCoef(distCoef.clone()), mbf(bf), mThDepth(thDepth)
    {
        // Frame ID
        mId = make_pair(nNextId++, ClientId);
    ...
    void Frame::ComputeBoW()
    {
        if (mBowVec.empty())
        {
            ...
            mpORBvocabulary->transform(vCurrentDesc, mBowVec, mFeatVec, 4); // mBoWVec是词袋向量，mFeatVec是特征向量
        }
    }
