...
namespace cslam
{
    ...
    Map::Map(ros::NodeHandle Nh, ros::NodeHandle NhPrivate, size_t MapId, eSystemState SysState)
        : mnMaxKFid(0), mnMaxMPid(0),
          ...
          mSysState(SysState),
          mbLockMapUpdate(false), mbLockPointCreation(false), mnLastKfIdUnique(0), mbStopGBA(false), mbRunningGBA(false), mbFinishedGBA(false),
#ifdef FINALBA
    ...
    Map::Map(const mapptr &pMapTarget, const mapptr &pMapToFuse)
        : mNh(pMapTarget->mNh), mNhPrivate(pMapTarget->mNhPrivate),
          ...
          mbLockMapUpdate(false), mbLockPointCreation(false), mnLastKfIdUnique(pMapToFuse->GetLastKfIdUnique()), mbStopGBA(false), mbRunningGBA(false), mbFinishedGBA(false),
#ifdef FINALBA
          mbGBAinterrupted(false),
#endif
          mbNoStartGBA(false), mbStopGBAInProgress(false)
    ...
    void Map::LoadMap(const string &path_name, vocptr voc, commptr comm, dbptr kfdb, uidptr uid)
    {
        ...
        for (unsigned long int i = 0; i < filenames_kf.size(); i++)
        {
            ...
            if (fs.is_open())
            {
                ...
                if (kf->mId.second == 0)
                    next_kf_id0 = max(next_kf_id0, kf->mId.first + 1);
                //            this->AddKeyFrame(kf);
                if (kf->mId.first == 0)
                    mvpKeyFrameOrigins.push_back(kf);
                fs.close();
            }
            else
            {
        ...
        for (unsigned long int i = 0; i < filenames_mp.size(); i++)
        {
            ...
            if (fs.is_open())
            {
                ...
                if (mp->mId.second == 0)
                    next_mp_id0 = max(next_mp_id0, mp->mId.first + 1);
                //            this->AddMapPoint(mp);
                fs.close();
            }
            else
        ...
        for (auto kf : keyframes)
        {
            for (auto mit = kf->mmMapPoints_minimal.begin(); mit != kf->mmMapPoints_minimal.end(); ++mit)
            {
                ...
                if (!lm)
                    std::cout << COUTWARN << "requested LM " << lm_id.first << "|" << lm_id.second << " does not exist" << std::endl;
    ...
    void Map::MapTrimming(kfptr pKFcur)
    {
        ...
        for (map<idpair, mpptr>::iterator mit = mmpMapPoints.begin(); mit != mmpMapPoints.end();)
        {
            ...
            if (bErase || pMPi->isBad())
            {
                ...
                if (pMPi->mId.second == mMapId)
                    mmpErasedMapPoints[pMPi->mId] = pMPi;
    ...
    void Map::PackVicinityToMsg(kfptr pKFcur, ccmslam_msgs::Map &msgMap, ccptr pCC)
    {
        ...
        for (int it = 1; it <= depth; ++it)
        {
            ...
            for (set<kfptr>::iterator sit = spKFsAddedLastIteration.begin(); sit != spKFsAddedLastIteration.end(); ++sit)
            {
                kfptr pKFi = *sit;
                if (pKFi->mId.first == 0)
                    continue; // we do not send the origin KFs
    ...
    void Map::RequestBA(size_t nClientId)
    {
        ...
        if (msnFinishedAgents.size() == msuAssClients.size())
        {
            ...
            for (set<ccptr>::iterator sit = mspCC.begin(); sit != mspCC.end(); ++sit)
            {
                ...
                if (pCC->mClientId > 3)
                    cout << "\033[1;31m!!! ERROR !!!\033[0m In \"LoopFinder::CorrectLoop()\": associated ClientId out of bounds (" << pCC->mClientId << ")" << endl;
                if (!(this->msuAssClients.count(pCC->mClientId)))
                    cout << "\033[1;31m!!! ERROR !!!\033[0m In \"LoopFinder::CorrectLoop()\": associated ClientId in pCC but not in msuAssClients" << endl;
                switch (pCC->mClientId)
                {
                case (static_cast<size_t>(0)):
                    if (b0)
                        cout << "\033[1;31m!!! ERROR !!!\033[0m In \"LoopFinder::CorrectLoop()\": associated ClientId found twice" << endl;
                    b0 = true;
#ifdef LOGGING
                ...
                case (static_cast<size_t>(3)):
                    ...
                    break;
                default:
                    cout << "\033[1;31m!!! ERROR !!!\033[0m In \"LoopFinder::CorrectLoop()\": associated ClientId out of bounds" << endl;
                }
...
// If optimized by Global BA, just update
                ...
                else
                {
                    ...
                    {
                        std::cout << COUTERROR << " KF" << pRefKF->mId.first << "|" << pRefKF->mId.second << ": !(pRefKF->mTcwBefGBA.dims >= 2)"
                                  << " -- bad: " << (int)pRefKF->isBad() << std::endl;
